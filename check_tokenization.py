...
STANZA_PIPELINE = stanza.Pipeline('en',
        processors='tokenize,lemma',tokenize_no_ssplit=True)
...
def main():
  ...
  for sentence in tqdm.tqdm(corpus):
    ...
    stanza_counter.update([token.to_dict()[0]['lemma'] for token in sentence.tokens])

  with open("vocabs.json", 'w') as f:
    json.dump({"stanza_vocab":stanza_counter,
              "bert_vocab":bert_counter}, f)
